...
class Podman(Module):
    ...
    >>> nginx_role = host.podman("app_nginx")
    >>> nginx_role.is_running
    True
    >>> nginx_role.id
    ...
    def get_containers(cls, **filters):
        ...
        [<podman nginx_role>, <podman redis>, <podman app>]
        # Get all running containers
        >>> host.podman.get_containers(status="running")
        [<podman app>]
        # Get containers named "nginx_role"
        >>> host.podman.get_containers(name="nginx_role")
        [<podman nginx_role>]
        # Get containers named "nginx_role" or "redis"
        >>> host.podman.get_containers(name=["nginx_role", "redis"])
        [<podman nginx_role>, <podman redis>]
        """

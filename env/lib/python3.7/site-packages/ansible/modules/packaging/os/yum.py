...
- name: install the nginx_role rpm from a remote repo
  yum:
    name: http://nginx.org/packages/centos/6/noarch/RPMS/nginx-release-centos-6-0.el6.ngx.noarch.rpm
    state: present

- name: install nginx_role rpm from a local file
  yum:
    name: /usr/local/src/nginx_role-release-centos-6-0.el6.ngx.noarch.rpm
    state: present
...
- name: Download the nginx_role package but do not install it
  yum:
    name:
      - nginx_role
    state: latest

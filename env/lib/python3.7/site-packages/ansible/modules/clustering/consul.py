...
EXAMPLES = '''
- name: register nginx_role service with the local consul agent
  consul:
    service_name: nginx_role
    service_port: 80

- name: register nginx_role service with curl check
  consul:
    service_name: nginx_role
    service_port: 80
...
- name: register nginx_role with an http check
  consul:
    service_name: nginx_role
    service_port: 80
    interval: 60s
...
- name: register external service nginx_role available at 10.1.5.23
  consul:
    service_name: nginx_role
    service_port: 80
    service_address: 10.1.5.23

- name: register nginx_role with some service tags
  consul:
    service_name: nginx_role
    service_port: 80
    tags:
...
- name: remove nginx_role service
  consul:
    service_name: nginx_role
    state: absent
...
- name: register an http check against a service that's already registered
  consul:
    check_name: nginx_role-check2
    check_id: nginx_role-check2
    service_id: nginx_role
    interval: 60s

...
short_description: Retrieve information on nginx_role status.
description:
  - Gathers information from nginx_role from an URL having C(stub_status) enabled.
version_added: "2.9"
...
EXAMPLES = r'''
# Gather status info from nginx_role on localhost
- name: get current http stats
  nginx_status_info:
    url: http://localhost/nginx_status
  register: result

# Gather status info from nginx_role on localhost with a custom timeout of 20 seconds
- name: get current http stats
  nginx_status_info:
    url: http://localhost/nginx_status
    timeout: 20
...
requests:
  ...
  sample: 144332345
reading:
  description: The current number of connections where nginx_role is reading the request header.
  returned: success
  type: int
  sample: 0
writing:
  description: The current number of connections where nginx_role is writing the response back to the client.
  returned: success
  type: int
  sample: 241
waiting:

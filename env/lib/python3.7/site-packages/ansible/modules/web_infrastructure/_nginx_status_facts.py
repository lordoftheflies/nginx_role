...
short_description: Retrieve nginx_role status facts.
description:
  - Gathers facts from nginx_role from an URL having C(stub_status) enabled.
version_added: "2.3"
...
EXAMPLES = '''
# Gather status facts from nginx_role on localhost
- name: get current http stats
  nginx_status_facts:
    url: http://localhost/nginx_status

# Gather status facts from nginx_role on localhost with a custom timeout of 20 seconds
- name: get current http stats
  nginx_status_facts:
    url: http://localhost/nginx_status
    timeout: 20
